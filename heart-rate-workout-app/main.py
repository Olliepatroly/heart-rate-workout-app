...
from ant_manager_133 import ANTManager
...
class HeartRateMonitorApp(App):
    ...
    def start_scan(self, instance):
        """Start scanning for devices based on connection type"""
        # Replace your existing start_scan method with this one
        try:
            ...
            # Start scan with the active manager instead of bt_manager directly
            self.active_manager.start_scan()  # Changed from self.bt_manager.start_scan()
            
            # Check for devices after a delay to allow scan to complete
            Clock.schedule_once(self.update_device_list, 10)
    ...
    def update_device_list(self, dt):
        ...
        # Replace your existing update_device_list method with this one
        try:
            # Get devices from the active manager instead of bt_manager directly
            devices = self.active_manager.get_discovered_devices()
            
            if devices:
                # Format depends on manager type
                if self.connection_type == 'bluetooth':
                    # Original BLE formatting
                    device_list = [f"{d['name']} ({d['address']})" for d in devices]
                else:  # ANT+
                    # ANT+ formatting (ID format is different)
                    device_list = [f"{d['name']} (ID: {d['id']})" for d in devices]
    ...
    def connect_selected_device(self, instance):
        ...
        # Replace your existing connect_selected_device method with this one
        if not self.device_spinner.values:
            return
            
        selected = self.device_spinner.text
        if selected == "No devices found":
            return
        
        # Extract device identifier based on connection type
        if self.connection_type == 'bluetooth':
            # BLE: Extract address from "Name (address)"
            identifier = selected.split("(")[1].split(")")[0]
        else:  # ANT+
            # ANT+: Extract ID from "Name (ID: id)"
            identifier = selected.split("ID: ")[1].split(")")[0]
        
        # Connect to the device with the active manager instead of bt_manager directly
        self.connect_btn.text = "Connecting..."
        self.connect_btn.disabled = True
        self.active_manager.connect(identifier)  # Changed from self.bt_manager.connect()
