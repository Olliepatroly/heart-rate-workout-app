...
specifically for heart rate monitors. This version is updated based on the actual
OpenANT 1.3.3 API structure and logging output.
"""
...
class ANTManager(EventDispatcher):
    ...
    def _scan(self):
        ...
        try:
            ...
            # The appropriate driver will be selected automatically
            try:
                # Create a node with no arguments - the proper driver will be auto-selected
            ...
            channel.set_search_timeout(30)  # 30 seconds timeout
            ...
            def on_device_found(data):
                ...
                try:
                    ...
                    if hr_value is not None:
                        # For demonstration, create a simple device entry
                        device_number = len(self.discovered_devices) + 1
                        
                        # Only add if this appears to be a new device based on HR value
                        if not self.discovered_devices or abs(hr_value - self.discovered_devices[-1].get('last_hr', 0)) > 3:
                            device_info = {
            ...
            # Wait for devices to be found (30 seconds max)
            timeout = time.time() + 30
            while time.time() < timeout and self.scanning:
                if hr_found and len(self.discovered_devices) >= 1:
                    # We found at least one HR monitor, but keep scanning for a bit
                    if time.time() > timeout - 20:  # Scan for at least 10 seconds
                        break
                time.sleep(0.1)
