#!/usr/bin/env python3
"""
ANT+ Tester Application (Custom Version for OpenANT 1.3.3)

A simple Kivy application to test ANT+ USB stick detection and openant functionality.
This version is customized for OpenANT 1.3.3 which has a different module structure.
"""
...
class ANTDeviceScanner:
    ...
    def find_ant_usb_devices(self):
        ...
        This method is customized for OpenANT 1.3.3 which doesn't have USBDevice class.
        """
        devices = []
        ...
        try:
            ...
            for vendor_id, product_id in ant_ids:
                ...
                if device:
                    ...
                    # Create a simple device object that Node can use
                    # For OpenANT 1.3.3, we'll use a simple class to mimic the USBDevice behavior
                    class SimpleUSBDevice:
                        ...
                        def __str__(self):
                            return f"ANT+ USB Device ({self.vendor_id:04x}:{self.product_id:04x})"
                    
                    ant_device = SimpleUSBDevice(vendor_id, product_id)
                    devices.append(ant_device)
                    
                    # We only need one device for now
                    break
    ...
    def _scan_worker(self):
        ...
        try:
            ...
            # Get USB device using our custom finder
            ...
            # For OpenANT 1.3.3, we need to directly use the PyUSB device
            # instead of the USBDevice class
            self.log_message("Connecting to ANT+ stick...")
            
            try:
                # Special handling for OpenANT 1.3.3
                # Instead of creating a Node with the device, we'll use the raw USB device
                import usb.core
                import usb.util
                
                # See if we can create a driver from the device
                self.log_message("Creating ANT+ driver...")
                try:
                    from openant.base.ant import Driver
                    from openant.base.commons import format_list
                    driver = Driver(device.device)
                    self.log_message("ANT+ driver created successfully")
                    
                    # Create the node with the driver
                    self.node = Node(driver)
                    self.log_message("ANT+ node created successfully")
                except Exception as e:
                    self.log_message(f"Error creating ANT+ driver: {e}")
                    traceback.print_exc()
                    raise e
                
                self.log_message("Starting ANT+ node...")
                self.node.start()
                self.log_message("ANT+ node started successfully")
                
                # Set network key
                self.log_message("Setting network key...")
                network = self.node.new_network(key=ANTPLUS_NETWORK_KEY)
                
                # Get some device-specific info if possible
                try:
                ...
                self.log_message(f"Error setting up ANT+ connection: {e}")
                traceback.print_exc()
...
class ANTTesterApp(App):
    ...
    def build(self):
        self.title = "ANT+ Tester App (Custom for OpenANT 1.3.3)"
        Window.size = (800, 600)
        ...
        self.openant_status_label = Label(
            text=f"Status: {openant_status} (Version: {getattr(openant, '__version__', 'unknown')})",
            color=openant_color,
            size_hint_y=0.6,
            halign='left',
            text_size=(Window.width, None)
        ...
        self.usb_path_label = Label(
            text="Checking for ANT+ USB devices...",
            size_hint_y=0.6,
            halign='left',
            text_size=(Window.width, None)
        )
    ...
    def check_usb_devices(self, dt):
        ...
        try:
            # Use PyUSB directly to look for ANT+ devices
            import usb.core
            ...
            for vendor_id, product_id in ant_ids:
                ...
                if device:
                    found_device = True
                    self.usb_path_label.text = f"Found ANT+ USB device: {vendor_id:04x}:{product_id:04x}"
                    self.usb_path_label.color = [0.2, 0.8, 0.2, 1]
                    self.update_log(f"Found ANT+ USB device: {vendor_id:04x}:{product_id:04x}")
            ...
            self.usb_path_label.text = f"Error checking USB devices: {e}"
            self.usb_path_label.color = [0.8, 0.2, 0.2, 1]
